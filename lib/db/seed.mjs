...
const BUSINESSES = [
  {
    ...
    services: ['M&A Advisory', 'Financial Due Diligence', 'Market Entry Strategy', 'Investor Relations', 'Restructuring & Turnaround'],
  },
  {
    ...
    services: ['Temperature-Controlled Freight', 'Last-Mile Delivery', 'Customs Brokerage', 'Supply Chain Consulting', 'Warehousing & Distribution'],
  },
  {
    ...
    services: ['Solar Farm Development', 'Battery Storage Systems', 'Energy PPAs', 'Grid Integration', 'Feasibility Studies'],
  },
  {
    name: 'Helix BioTech',
    industry: 'healthcare',
    ...
    services: ['mRNA Therapy Development', 'Genetic Diagnostics', 'Clinical Trial Partnerships', 'Biomarker Research', 'Licensing & IP'],
  },
  {
    ...
    services: ['Revenue Intelligence Platform', 'CRM Automation', 'Sales Forecasting AI', 'API Integrations', 'Enterprise Licensing'],
  },
  {
    ...
    services: ['Precision CNC Machining', 'Robotics Sub-Assembly', 'Semiconductor Parts', 'OEM Contract Manufacturing', 'Quality Certification'],
  },
  {
    ...
    services: ['Organic Soybean Export', 'Specialty Coffee Supply', 'Tropical Fruit Export', 'Organic Certification', 'Bulk Commodity Trading'],
  },
  {
    ...
    services: ['Cross-Border M&A', 'IP Licensing', 'Trade Compliance', 'Dispute Resolution', 'Corporate Governance'],
  },
  {
    ...
    services: ['Managed SOC', 'Zero-Trust Architecture', 'Cloud Security Audit', 'Compliance Automation', 'Penetration Testing'],
  },
  {
    ...
    services: ['Wholesale Distribution', 'Brand Licensing', 'Retail Concession Agreements', 'Visual Merchandising', 'Pop-Up Retail Spaces'],
  },
  {
    ...
    services: ['Private Equity Co-Investment', 'Real Estate Debt', 'Infrastructure Funds', 'LP Advisory', 'Fund Structuring'],
  },
  {
    ...
    services: ['Surgical Robotics', 'Remote Patient Monitoring', 'Medical AI Software', 'Hospital Integration', 'Regulatory Affairs Support'],
  },
  {
    ...
    services: ['Sea Freight Brokerage', 'Air Cargo', '3PL Fulfilment', 'Real-Time Freight Marketplace', 'Import / Export Documentation'],
  },
  {
    name: 'Volta Power Co.',
    industry: 'energy',
    ...
    services: ['Mini-Grid Development', 'Hybrid Solar-Diesel Systems', 'Rural Electrification', 'Carbon Credits', 'Energy Access Financing'],
  },
  {
    ...
    services: ['Quantum Algorithm Licensing', 'Photonic Chip Design', 'Drug Discovery Simulation', 'Financial Risk Modelling', 'R&D Partnerships'],
  },
]
  ...
  try {
    ...
    for (const biz of BUSINESSES) {
      ...
      if (existing.rows.length > 0) {
        ...
        // Update services for existing businesses
        await client.query(
          `UPDATE businesses SET services = $1 WHERE id = $2`,
          [biz.services ?? [], existing.rows[0].id]
        )
        console.log(`  updated services: ${biz.name}`)
        continue
      }
      ...
      const res = await client.query(
        `INSERT INTO businesses (name, slug, industry, country, city, website, description, services, verification_status)
         ...
         RETURNING id`,
        [biz.name, slug, biz.industry, biz.country, biz.city, biz.website, biz.description, biz.services ?? []]
      )
    ...
    // System admin user — needs a business_id due to NOT NULL constraint; use a platform internal business
    const platformBiz = await client.query(
      `INSERT INTO businesses (name, slug, industry, country, verification_status)
       VALUES ('NexusB2B Platform', 'nexusb2b-platform', 'technology', 'US', 'verified')
       ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name
       RETURNING id`
    )
    const platformBizId = platformBiz.rows[0].id

    const adminHash = await bcrypt.hash('admin1234', 12)
    await client.query(
      `INSERT INTO users (business_id, email, name, password_hash, role, is_active, email_verified)
       VALUES ($1, 'admin@nexusb2b.io', 'Platform Admin', $2, 'system_admin', true, true)
       ON CONFLICT (email) DO UPDATE
         SET password_hash  = EXCLUDED.password_hash,
             role           = 'system_admin',
             business_id    = EXCLUDED.business_id,
             is_active      = true,
             email_verified = true`,
      [platformBizId, adminHash]
    )
    console.log(`System admin upserted: admin@nexusb2b.io / admin1234`)
