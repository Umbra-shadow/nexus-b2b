...
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  await query(`UPDATE receipts SET status = 'acknowledged' WHERE id = $1`, [params.id])
  return NextResponse.redirect(new URL(`/receipts/${params.id}`, process.env.NEXT_PUBLIC_APP_URL!))
}
